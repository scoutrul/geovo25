<template>
    <BaseContainer :as="'section'" :bg="'bg-white-90'">
        ...
        <div class="flex w-full sm:grid flex-row sm:flex-col overflow-auto gap-6 sm:gap-8 sm:grid-cols-2 lg:grid-cols-4 pl-4 pr-4 ml-[-16px] mr-[-16px] min-w-[calc(100vw)] sm:min-w-[auto] sm:pl-0 sm:pr-0 sm:ml-0 sm:mr-0">
            <ExpertiseCard class="min-w-[212px] sm:min-w-[auto]" v-for="(card, index) in cards" :key="index"
...
defineProps({
    ...
    subtitle: {
        type: String,
        default: 'Получите не просто дизайнера, а партнера, который заботится о бизнесе и совмещает функции целой команды'
    },
    cards: {
        type: Array,
        required: true,
